...
const SingleBlogPage = async ({ params }) => {
  ...
  // 👇 The Verified URLs ensure no 404s reach the browser
  const seoImageUrl = getSeoUrl(metadata.featuredImage.url);
  ...
  const authorName = metadata.author?.name || "Shivansh";
  const authorUrl = metadata.author?.url || "https://saharanpurprice.in/about";
  ...
  const jsonLd = {
    ...
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": `https://saharanpurprice.in/blogs/${decodedSlug}`,
    },
    ...
    publisher: {
      ...
      logo: {
        "@type": "ImageObject",
        url: "https://saharanpurprice.in/icon.png",
      },
    },
    description: metadata.summary,
  };
